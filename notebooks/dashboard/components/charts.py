...
def get_last_laps(session, race_session, Lap_Number, quali):
    if race_session == 'Race' or race_session == 'Sprint':
        ...
        if drivers_data_df.empty:
            ...
            drivers_data_df.drop(columns=['Position'], inplace=True)
            styled_df = fmt.style_lap_df(drivers_data_df, session, Lap_Number)
        ...
        fastest_laps['Tyre'] = fastest_laps.Tyre.replace(to_replace=dt.tyres)
...
def get_lap_and_tyre(session, Lap_Number):
...
def get_race_control_message(session, race_session, Lap_Number):
...
def get_weather_info(session, Lap_Number):
...
def get_team_radio_chart(session, year, race_number):
    st.dataframe(dt.get_team_radio(session, year, race_number))

def get_last_laps_per_driver(session, Lap_Number, driver):
...
def get_track_status(session, Lap_Number):
    on_track = []
    on_track_df = []
    on_track_logo = []
...
def get_map(session, Lap_Number):
    ...
    align-items: center;
    ">
        <img src="data:image/png;base64,{image_base64}" style="max-height: 400px; object-fit: cover;" />
    </div>
    """
